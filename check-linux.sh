#!/usr/bin/env bash
# ──────────────────────────────────────────────────────────────────────────────
# axios supply-chain checker (Linux)
#
...
preflight() {
  ...
  # Raise file descriptor limit if needed
  ulimit -n 10240 2>/dev/null || ulimit -n 4096 2>/dev/null || true

  # Hard requirements
  if [[ "$(uname)" != "Linux" ]]; then
    bail "This script requires Linux"
  fi
  local distro_info
  distro_info=$(grep -s "^PRETTY_NAME=" /etc/os-release 2>/dev/null | cut -d'"' -f2 || echo "unknown")
  ok "Running on Linux (${distro_info})"
  ...
  local missing_tools=()
  for cmd in git npm sha1sum pgrep ss; do
...
check_system() {
  ...
  if [ -f "/tmp/ld.py" ]; then
    found "RAT dropper found: /tmp/ld.py"
  else
    ok "No dropper at /tmp/ld.py"
  fi

  # Also check for macOS artefact in case of shared filesystem
  trace "Checking for macOS RAT artefact ..."
  if [ -f "/tmp/com.apple.act.mond" ]; then
    found "Suspicious file found: /tmp/com.apple.act.mond"
  fi

  section "Persistence checks (systemd)"
  ...
  for sd in "${systemd_dirs[@]}"; do
    trace "Scanning ${sd} ..."
    if [ -d "$sd" ] && [ -r "$sd" ]; then
      local hits
      hits=$(grep -rl "sfrclak\|142\.11\.206\.73\|ld\.py\|plain-crypto-js" "$sd" 2>/dev/null || true)
      if [ -n "$hits" ]; then
        found "Suspicious systemd unit referencing IOC: ${hits}"
      else
        ok "No suspicious entries in ${sd}"
      fi
    elif [ -d "$sd" ]; then
      warn "Cannot read ${sd} (permission denied) — run with sudo for full check"
    else
      trace "${sd} does not exist — skipping"
    fi
  done

  section "Persistence checks (cron)"

  trace "Checking user crontab ..."
  local cron_output
  cron_output=$(crontab -l 2>/dev/null || true)
  if [ -n "$cron_output" ]; then
    if echo "$cron_output" | grep -qi "sfrclak\|142\.11\.206\.73\|ld\.py\|plain-crypto-js"; then
      found "Suspicious entry in user crontab referencing IOC"
    else
      ok "No suspicious entries in user crontab"
    fi
  else
    ok "No user crontab set"
  fi

  local cron_dirs=("/etc/crontab" "/etc/cron.d")
  for cd in "${cron_dirs[@]}"; do
    if [ -e "$cd" ] && [ -r "$cd" ]; then
      trace "Scanning ${cd} ..."
      if grep -r "sfrclak\|142\.11\.206\.73\|ld\.py\|plain-crypto-js" "$cd" 2>/dev/null | grep -q .; then
        found "Suspicious cron entry referencing IOC in ${cd}"
      else
        ok "No suspicious entries in ${cd}"
      fi
    fi
  done

  section "Persistence checks (shell rc files)"
  ...
  for rc in "${rc_files[@]}"; do
    if [ -f "$rc" ]; then
      trace "Scanning ${rc} ..."
      if grep -q "sfrclak\|142\.11\.206\.73\|/tmp/ld\.py\|plain-crypto-js" "$rc" 2>/dev/null; then
        found "Suspicious entry in ${rc} referencing IOC"
      else
        ok "No suspicious entries in ${rc}"
      fi
    fi
  done
  ...
  if command -v pgrep &>/dev/null; then
    trace "Looking for suspicious RAT processes ..."
    local pgrep_hit=0
    if pgrep -f "ld\.py" > /dev/null 2>&1; then
      found "Process matching 'ld.py' is RUNNING"
      pgrep_hit=1
    fi
    if pgrep -f "sfrclak" > /dev/null 2>&1; then
      found "Process referencing C2 domain 'sfrclak' is RUNNING"
      pgrep_hit=1
    fi
    if [ "$pgrep_hit" -eq 0 ]; then
      ok "No suspicious RAT processes running"
    fi
  else
  ...
  if command -v dig &>/dev/null; then
    ...
    if nslookup sfrclak.com 2>/dev/null | grep "Address:" | grep -qv "#53"; then
      warn "DNS resolution for sfrclak.com succeeded — C2 may still be active"
    else
      ok "C2 domain sfrclak.com does not resolve"
    fi
  else
    trace "dig/nslookup not available — skipping DNS check"
  fi

  # Active connections — prefer ss (no root needed), fall back to lsof
  trace "Checking for connections to C2 IP 142.11.206.73 ..."
  if command -v ss &>/dev/null; then
    if ss -tnp 2>/dev/null | grep -q "142.11.206.73\|sfrclak"; then
      found "Active connection to C2 infrastructure detected via ss!"
    else
      ok "No active connections to C2 infrastructure"
    fi
    ...
    warn "ss/lsof not available — skipping connection check"
  fi
}
  ...
  if [ -d "$npm_cache" ]; then
    ...
    if command -v sha1sum &>/dev/null; then
      ...
      while IFS= read -r tarball; do
        [ -z "$tarball" ] && continue
        local sha
        sha=$(sha1sum "$tarball" 2>/dev/null | awk '{print $1}' || true)
      ...
      warn "sha1sum not available — skipping tarball hash verification"
    fi
  else
...
check_global_node_modules() {
  ...
  if [ -d "$fnm_dir" ]; then
    ...
    done < <(find "$fnm_dir" -maxdepth 4 -type d -name "node_modules" -path "*/lib/node_modules" 2>/dev/null || true)
  fi
...
check_yarn_cache() {
  ...
  # Yarn v1 cache
  local yarn_v1_cache="$HOME/.cache/yarn"
...
check_pnpm_store() {
  ...
  # Fallback to common Linux location
...
# ── interactive directory picker (Linux — zenity/kdialog if available) ────────
pick_directory() {
  # Try zenity (GNOME) or kdialog (KDE) if a display server is available
  if [ -n "${DISPLAY:-}" ] || [ -n "${WAYLAND_DISPLAY:-}" ]; then
    if command -v zenity &>/dev/null; then
      local chosen
      chosen=$(zenity --file-selection --directory --title="Select a repository to scan" 2>/dev/null || true)
      if [ -n "$chosen" ]; then
        echo "${chosen%/}"
        return
      fi
      ...
      local chosen
      chosen=$(kdialog --getexistingdirectory "$HOME" 2>/dev/null || true)
      if [ -n "$chosen" ]; then
        echo "${chosen%/}"
        return
      fi
  ...
  warn "No GUI file picker available — use option 4 to type a path instead"
  echo ""
}
...
print_verdict() {
  ...
  if [ "$FOUND_ISSUES" -eq 0 ]; then
    ...
    echo -e "${RED}${BOLD}║                                                                       ║${RESET}"
    echo -e "${RED}${BOLD}║     ⚠️  REACH OUT TO YOUR SECURITY TEAM LEAD IMMEDIATELY  ⚠️            ║${RESET}"
    echo -e "${RED}${BOLD}║                                                                       ║${RESET}"
    echo -e "${RED}${BOLD}║     📞 Slack, call, or direct message — do this NOW                   ║${RESET}"
    echo -e "${RED}${BOLD}║                                                                       ║${RESET}"
    echo -e "${RED}${BOLD}╚═══════════════════════════════════════════════════════════════════════╝${RESET}"
    echo ""
...
guided_project_scan() {
  ...
  while [ "$keep_scanning" -eq 1 ]; do
    ...
    echo -e "  ${CYAN}3${RESET})  Pick a directory using file chooser (if available)"
    ...
    case "$choice" in
      ...
      2)
        ...
        ;;
      3)
        info "Opening file picker … select a repository folder."
        local target_dir
